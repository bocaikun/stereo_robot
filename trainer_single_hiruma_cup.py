...
from model.single_hiruma import single_hiruma_att
...
def main(
    ...
    if not os.path.exists('D:/logs'):
        os.mkdir('D:/logs')
    if not os.path.exists('D:/logs/single'):
        os.mkdir('D:/logs/single')
    if not os.path.exists('D:/logs/singl/{}'.format(training_id)):
        os.mkdir('D:/logs/single/{}'.format(training_id))
    if not os.path.exists('logs/single/{}/models'.format(training_id)):
        os.mkdir('D:/logs/single/{}/models'.format(training_id))

    # load model
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    print("use device:", device)
    model = single_hiruma_att(device)
    ...
    # start learning
    with open('D:/logs/single/{}/progress.csv'.format(training_id), 'w') as file:
        header = 'epoch,time,train/pos,train/rec_img,test/pos,test_img\n'
        file.write(header)
    ...
    for e in range(epoch):
        ...
        for train_left, train_right, train_position in train_dataloader:
            model.train()
            train_loss, train_position_loss, train_left_loss = 0., 0., 0.
            ...
            for steps in range(49+3):
                ...
                t_pred_left, t_train_position, att_map1, train_rnn_hidden = model(t_train_left, t_train_position, train_rnn_hidden)
            ...
            train_img_loss = train_left_loss.item()
            
            train_loss = train_position_loss + train_left_loss
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
        ...
        if e % 10 == 0:
                with torch.no_grad():
                    for test_left, test_right,test_position in test_dataloader:
                        model.eval()
                        test_loss, test_position_loss, test_left_loss = 0., 0., 0.
                        ...
                        for steps in range(49+3):
                            ...
                            t_pred_left, t_test_position, t_test_attmap1, test_rnn_hidden = model(t_test_left, t_test_position, test_rnn_hidden)
                        ...
                        test_img_loss = test_left_loss.item()

                    w_line = '{},{:.4f},{:.16f},{:.16f},{:.16f},{:.16f}'.format(
                            e,  
                    ...
                    p_line = '{}, time:{:.4f}, train/pos_loss:{:.8f},train/red_loss:{:.8f}, test/pos_loss:{:.8f}, test/rec_loss:{:.8f}'.format(
                            e,  
                            elapsed_time.total_seconds(),
                            train_position_loss.item(), 
                            train_img_loss,
                    ...
                    with open('D:/logs/single/{}/progress.csv'.format(training_id), 'a') as f:
                        w_line += '\n'
                        f.write(w_line)

        if e % 50 == 0:
            print('Saving Models...')
            torch.save(model.state_dict(), 'D:/logs/single/{}/models/model_{}.pth'.format(training_id,e))
    ...
    #save model
    print('Saving Models...')
    torch.save(model.state_dict(), 'D:/logs/single/{}/models/model_final.pth'.format(training_id))   
    ...
    with open('D:/logs/single/{}/train_config.json'.format(training_id), 'w') as f:
        json.dump(config, f)
